...
def ModificarVenta(request):
    if request.method=='POST':
        ...
        form_devolucion=Form_ModificarVenta_Devolucion()
        ...
        return render(request, 'kadoshapp/ModificarVenta.html', {'form_persona':form_persona,  'form_Venta':form_Venta, 'form_Cliente':form_Cliente, 'form_DetalleVenta':form_DetalleVenta, 'form_empleado':form_empleado,'form_producto':form_producto,'form_inventario':form_inventario,'form_busquedas':form_busquedas,'form_devolucion':form_devolucion })
    else:
        form_Venta=Form_ModificarVenta_Venta()
        form_Cliente=Form_ModificarVenta_Cliente()
        form_DetalleVenta=Form_ModificarVenta_DetalleVenta()
                ...
                try:
                    ...
                    if detviejo:
                        if int(detviejo.cantidad_venta) != int(cant_venta): #se deben convertir a int, si no la comparación nunca ocurre; se compara si la cantidad guardada en ese detalle conicide con la cantidad de producto que se le dio al cliente en sustitución a lo que devolvió
                            cantmodificada=cant_venta
                            if int(cant_venta)>int(detviejo.cantidad_venta):
                                cantmodificada=detviejo.cantidad_venta
                            detalleviejo=DetalleVenta.objects.filter(pk=cod_detalle).update(  valor_parcial_venta=(F('valor_parcial_venta')-(cantmodificada*(F('valor_parcial_venta')/F('cantidad_venta')) )) ) #se actualiza el detalle de venta anterior, primero el valor
                            detalleviejo2=DetalleVenta.objects.filter(pk=cod_detalle).update(  cantidad_venta=(F('cantidad_venta')-cantmodificada)) #se actualiza el detalle de venta anterior, segundo la cantidad de venta
                    ...
                    nuevadevolucion=Devolucion(venta_idventa=Venta(pk=cod_venta),motivo_devolucion=motivo)
