...
def BuscarCaja(request):
    if request.method == 'POST':
        ...
        cheque=Venta.objects.filter(estado_venta=1,es_cotizacion=0,tipo_pago_idtipo_pago=TipoPago(pk=3),fecha_venta__range=(hoy_min,hoy_max),caja_idcaja=Caja(pk=caja)).values('caja_idcaja__pk').annotate(total_cheque=Sum('total_venta'))
